...
@Component({
  selector: 'app-digidoc-scanner',
  templateUrl: './digidoc-scanner.component.html',
  styleUrls: ['./digidoc-scanner.component.css', './digidoc-scanner.component.scss'],
  providers: [DigidocService],
  animations: [ adminpAnimations ]
})
export class DigidocScannerComponent implements OnInit {
  ...
  async getDataResults(aResult: Array<any>) {
    ...
    aResult.forEach( (x: any, index: number) => {
      ...
      if ( aCode !== '' ) {
        ...
        if ( nIdPersonal_.includes(nIdPersonal) === false ) {
          nIdPersonal_ = nIdPersonal_ + nIdPersonal + ',';
        }
    ...
    nIdPersonal_ = (nIdPersonal_.length > 0 ) ?  nIdPersonal_.substring(0, nIdPersonal_.length - 1) : '';
    nIdDocumento_ = (nIdDocumento_.length > 0 ) ?  nIdDocumento_.substring(0, nIdDocumento_.length - 1) : '';

    if ( nIdPersonal_.length > 0 && nIdDocumento_.length > 0 ) {

      const param = [];
      param.push('1¡A.nIdPersonal!' + nIdPersonal_ + '-1¡nIdDocumento!' + nIdDocumento_ );
      ...
      await this.service._loadSP( 7, param).then( (value: any[]) => {
        Object.values ( value ).forEach( (lista: Array<any>, iLista: number) => {
          switch (iLista) {
            // Personal
            case 0:
              this.aDataPersonal = lista;
              break;

            // Documento
            case 1:
              this.aDataDocument = lista;
              break;

            // Documentos del personal
            case 2:
              this.aDataDocPer = lista;
              break;
          }
        });
      });
    ...
    if (nGroup > -1) {
      ...
      if ( aCode !== '' ) {
        ...
        if (fg_nGroup !== nGroup) {
          ...
          const iPersonal = this.aDataPersonal.findIndex( x => x.nIdPersonal === Number(nIdPersonal) );
          ...
          this.fgInfoDoc.patchValue({
            ...
            sDocumento: this.aDataDocument[iDocument].sDesc,
            nIdPersonal: nIdPersonal,
            sNombres: this.aDataPersonal[iPersonal].sNombres,
            dFechIni: dFechIni,
            dFechFin: dFechFin,
            sPag: sPag,
            bValid: aFind.valid,
    ...
    for (let index = 0; index < aGroup.length; index++) {
      ...
      if ( element > 0 ) {
        ...
        if ( bValid === true ) {
          ...
          const sFechIni = aCode.split('|')[3] as string;
          const aFechIni = sFechIni.split('/');
          const dFechIni = aFechIni[1] + '/' + aFechIni[0] + '/' + aFechIni[2];

          const sFechFin = aCode.split('|')[4] as string;
          const aFechFin = sFechFin.split('/');
          const dFechFin = aFechFin[1] + '/' + aFechFin[0] + '/' + aFechFin[2];
          ...
          if ( nOption === 0 || nOption === 2284 || nOption === 2283 ) {
            ...
            param.push('T1¡nIdDocumento!' + nIdDocumento);
