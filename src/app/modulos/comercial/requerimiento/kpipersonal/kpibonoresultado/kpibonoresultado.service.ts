...
export class KpiiServiceBonoResultado {
  ...
  async _loadPersonal(nIdResp: number, fecha: string) {
    try {
      const response = await this.http.get(`${this.urlBase2}Incentivo/GetPersonalIncentivo?nIdResp=${nIdResp}&fecha=${fecha}`,
  ...
  async _loadSP(pOpcion: number, pParametro: any, url: string) {
    const urlEndPoint = url + 'AdminComercialService/GetKpiBonoR';
  ...
  async _crudKI(pOpcion: number, pParametro: any, url: string) {
    const urlEndPoint = url + 'AdminComercialService/GetCrudKBR';
    const httpHeaders = new HttpHeaders({ 'Content-Type': 'application/json' });
