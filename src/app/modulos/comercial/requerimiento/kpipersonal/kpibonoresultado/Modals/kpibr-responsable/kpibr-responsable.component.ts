...
import { KpiiServiceBonoResultado } from '../../../kpibonoresultado/kpibonoresultado.service';
import { kpiiAnimations } from './kpibr-responsable.animations';

@Component({
  selector: 'app-kpibr-responsable',
  templateUrl: './kpibr-responsable.component.html',
  styleUrls: ['./kpibr-responsable.component.css', 'kpibr-responsable.component.scss' ],
  animations: [kpiiAnimations],
  providers : [KpiiServiceBonoResultado],
})
export class KpibrResponsableComponent implements OnInit {
  @Input() fromParent;
  fgResp: FormGroup;
  ...
  private spi: NgxSpinnerService, public service: KpiiServiceBonoResultado, private _snackBar: MatSnackBar) {
    this.url = baseUrl;
    this.new_fgResp();
    this.new_fgInfoUser();
   }
