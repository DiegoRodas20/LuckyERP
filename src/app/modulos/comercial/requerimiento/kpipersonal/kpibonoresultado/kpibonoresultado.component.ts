...
import { KpiiServiceBonoResultado } from './kpibonoresultado.service';
import { kpiiAnimations } from './kpibonoresultado.animations';
...
import { KpibrBonoRComponent } from './Modals/kpibr-bonor/kpibr-bonor.component';
import { KpibrResponsableComponent } from './Modals/kpibr-responsable/kpibr-responsable.component';
...
const MODALS: { [name: string]: Type<any> } = {
  responsable: KpibrResponsableComponent,
  bonoR: KpibrBonoRComponent
};


@Component({
  selector: 'app-kpibonoresultado',
  templateUrl: './kpibonoresultado.component.html',
  styleUrls: ['./kpibonoresultado.component.css','./kpibonoresultado.component.scss'],
  providers: [ KpiiServiceBonoResultado ],
  animations : [ kpiiAnimations ]
})
export class KpibonoresultadoComponent implements OnInit {
//#region Variables
...
MainDS: MatTableDataSource<nsBonoR.IMain> = new MatTableDataSource([]);
...
ExpandedDS: MatTableDataSource<nsBonoR.IExpanded> = new MatTableDataSource([]);
@ViewChild('pagExpanded', {static: false}) pagExpanded: MatPaginator;
@ViewChild('mtExpanded', {static: false}) mtExpanded: MatTable<nsBonoR.IExpanded>;
aPerso: Array<nsBonoR.IPerso>;
...
// Array
aCentroCosto: nsBonoR.ICentroCosto[] = [];
aPersonal: nsBonoR.ITeamResp[] = [];
aPlanilla: nsBonoR.IPlanilla[] = [];
...
//#endregion
  ...
  constructor(public service: KpiiServiceBonoResultado, @Inject('BASE_URL') baseUrl: string,
    ...
    case 1:
      switch (index) {
        ...
        case 1:
          ...
          this.fgBonoR.controls['T1_nIdPlla'].setValue(aPlanilla[0].nIdPlla);
          this.fgBonoR.controls['sPlla'].setValue(aPlanilla[0].sDesc);
          this.showModal(1);
          break;
      }
      break;
...
openModal(name: string , fgDevengue: any, fgBonoR: any, aPersonal: any, aSaldoCC: any , 
...
async Validaciones(nIdEmp: number , sCodCC: string) {
  ...
  await this.service._ValidarAprobacionCC(nIdEmp, sCodCC).then((response: any) => {
    if (response !== undefined ) {
      ...
      this.fgBonoR.controls['T1_nIdCentroCosto'].setValue(value[0].nIdCentroCosto);
      this.fgBonoR.controls['nCentroCosto'].setValue(sCodCC);
      this.fgBonoR.controls['sCentroCosto'].setValue(value[0].sCentroCosto);
      this.fgBonoR.controls['sCliente'].setValue(value[0].sCliente);
      bReturn = false;
      } else {
        bReturn = true;
        Swal.fire(
...
async showModal(opc: number, pushParam?: any, index?: number) {
  ...
  this.fgBonoR.controls['T1_nIdResp'].setValue(nIdResp);
  this.fgBonoR.controls['sResp'].setValue(sResp);
  ...
  switch (opc) {
    ...
    case 1:
      ...
      .then( async resultado => {
        if (resultado.isConfirmed) {
          ...
          if (lError) {
            this._snackBar.open('N° de centro de costo sin saldo para bonos.', 'Cerrar', {
              horizontalPosition: 'right',
              verticalPosition: 'top',
              duration: 2500
            });
          ...
          this.fgBonoR.controls['T1_nIdPartida'].setValue(this.fcPartida.value);
          ...
          this.fgBonoR.controls['T1_nIdDevengue'].setValue(nIdDevengue);
          this.fgBonoR.controls['T1_dFecha'].setValue(dFecha);
          this.fgBonoR.controls['sEstado'].setValue('Temporal');

          this.toggleBono = 2;
          this.mBono = 1;
          // this.fgSearch.controls['sNombres'].enable();
          this.openModal('bonoR', this.fgDevengue, this.fgBonoR, this.aPersonal,
    ...
    case 2:
      ...
      const rElement = pushParam as nsBonoR.IMain;

      this.fgBonoR.controls['T1_nIdBonoR'].setValue(rElement.nIdBonoR);
      this.fgBonoR.controls['T1_nIdCentroCosto'].setValue(rElement.nIdCentroCosto);
      this.fgBonoR.controls['nCentroCosto'].setValue(rElement.nCentroCosto);
      this.fgBonoR.controls['sCentroCosto'].setValue(rElement.sCentroCosto);
      this.fgBonoR.controls['sCliente'].setValue(rElement.sCliente);
      this.fgBonoR.controls['T1_nIdPartida'].setValue(rElement.nIdPartida);
      this.fgBonoR.controls['T1_nIdDevengue'].setValue(rElement.nIdDevengue);
      this.fgBonoR.controls['T1_dFecha'].setValue(rElement.dFecha);
      this.fgBonoR.controls['T1_nIdEstado'].setValue(rElement.nIdEstado);
      this.fgBonoR.controls['sEstado'].setValue(rElement.sEstado);
      this.fgBonoR.controls['T1_nIdPlla'].setValue(rElement.nIdPlla);
      this.fgBonoR.controls['sPlla'].setValue(rElement.sPlla);
      this.fgBonoR.controls['T1_sFileSustento'].setValue(rElement.sFileSustento);

      await this.loadTeamResp(1, nIdResp as number);
      await this.loadBono(rElement.nIdBonoR);
...
async ngOnInit(): Promise<void> {
  ...
  let aInfoUser: Array<nsBonoR.IInfoUser>;
  ...
  await this.service._loadSP( 1, param, this.url).then( (value: any[]) => {
    Object.values ( value ).forEach( (lista: Array<any>, iLista: number) => {
      switch (iLista) {
        ...
        // Info User
        case 0:
          aInfoUser = lista as Array<nsBonoR.IInfoUser>;
        ...
        // Devengue
        case 1:
          const aDevengue = lista as Array<nsBonoR.IDevengue>;
        ...
        // Planilla
        case 2:
          this.aPlanilla = lista as Array<nsBonoR.IPlanilla>;
          break;
    ...
    await this.service._loadSP( 3, param, this.url).then( (value: any[]) => {
      Object.values ( value ).forEach( (lista: Array<any>, iLista: number) => {
        switch (iLista) {
          ...
          // Lista Main
          case 0:
            const aMain = lista as Array<nsBonoR.IMain>;
            ...
            this.MainDS.filterPredicate = ((data: nsBonoR.IMain, filter: any ) => {
              ...
              const e = this.cantPerso(data.nIdBonoR) > 0;
              return a && b && c && d && e;
          ...
          // Lista Expanded
          case 1:
            this.aPerso = lista as Array<nsBonoR.IPerso>;
            break;
        }
      });
    });
  ...
  async loadCentroCosto(sCodCC: string) {
    ...
    await this.service._loadSP( 4, param, this.url).then( (value: nsBonoR.ICentroCosto[]) => {
      if (value.length > 0) {
  ...
  async loadTeamResp(opc: number, nIdResp: number) {
    ...
    // param.push('10¡dFecha!' + sEjMes + '-10¡nIdResp!' + nIdResp);
    // sp_KpiBonoR
    // '-10¡dFecha!' + sEjMes + 
    ...
    if (!bReturn) {
      if (this.aPersonal.filter(x => x.sCodPlla === '1' || x.sCodPlla === '7' ).length > 0 ) {
        this.aPersonal = this.aPersonal.filter(x => x.sCodPlla === '1' || x.sCodPlla === '7' );
        bReturn = false;
      } else {
        Swal.fire(
          'Personal fuera de planilla asignada.',
          'El personal debe ser de planilla 1 o 7.',
          'info'
        );
        bReturn = true;
      }
  ...
  async loadBono(nIdBonoR: number) {
    const param = [];
    ...
    await this.service._loadSP( 7, param, this.url).then( (value: nsBonoR.IBonoR[]) => {
      if (value.length > 0) {
        ...
        this.BonoTrimetralDS.filterPredicate = ((data: nsBonoR.IBonoR, filter: any ) => {
          // tslint:disable-next-line: max-line-length
  ...
  async clickExpanded(row: nsBonoR.IMain) {
    ...
    if ( this.expandedMore === row ) {
      ...
      const param = [];
      param.push('0¡A.nIdBonoR!' + row.nIdBonoR);

      await this.service._loadSP( 6, param, this.url).then( (value: nsBonoR.IExpanded[]) => {
  ...
  async clickResp(element: nsBonoR.IListaResp) {
  ...
  uploadFile(event) {
    if (event.target.files[0]) {
      ...
      this.toggleBono = 3;
      this.delay(250).then(any => {
        this.onToggleFab(this.toggleBono, 1);
      });
