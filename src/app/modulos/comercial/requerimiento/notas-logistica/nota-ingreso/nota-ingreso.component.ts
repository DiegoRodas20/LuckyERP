...
export class NotaIngresoComponent implements OnInit {
  ...
  ListaNotaColumns: string[] =
    ['nIdOperMov', 'sNomPresupuesto', 'sDocumento', 'nCantidad', 'sFechaCreado', 'sFechaAgendado', 'sEstado'];
  ...
  async fnListarAnios() {
    ...
    let data = await this.notasLogisticaService.fnControlNotaIngreso
      (1, 2, pParametro, 7, pParametroDet, this.url) as string[];
    this.lAnios = data;
  ...
  crearEstados() {
    ...
    let lista: Estado[] = [
      ...
      { codigo: 2295, nombre: 'Agendado Logística' },
