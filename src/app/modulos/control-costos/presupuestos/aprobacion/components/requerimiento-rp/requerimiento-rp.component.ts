...
@Component({
  selector: 'app-requerimiento-rp',
  templateUrl: './requerimiento-rp.component.html',
  styleUrls: ['./requerimiento-rp.component.css'],
  animations: [asistenciapAnimations]
})
export class RequerimientoRpComponent implements OnInit {
  @Input() data: any;
  ...
  displayedColumns: string[] = ["sCentroCosto", "sSucursal", "sPartida", "sDepositario",
    "sCargo", "sHerramienta", "nUnidades", "nTotal"];
  ...
  async ngOnInit() {
    ...
    let lCabecera = await this.presupuestoService.listarDocumentosParaAprobacion(11, `${this.data.nIdGastoCosto}`);
    this.vRqHerramienta = lCabecera[0];
    this.listaDetalleCabecera = await this.presupuestoService.listarDocumentosParaAprobacion(12, `${this.data.nIdGastoCosto}`) as any;
    ...
    */
    switch (tipo) {
      case 1:
        ...
        if (respSwet.resp === 1) {
          resp = await this.presupuestoService.actualizarDocumentoAprobar(1, `${this.data.nIdGastoCosto}|${idEstado}|${idUser}|${pPais}|${this.data.nIdUserRegistro}|RP|${respSwet.observacion}|${pEmpresa}`);
          if (resp) {
  ...
  async actualizarMensaje(id: number, msj: string, estado: string) {
    ...
    if (msj === 'Aprobado') {
      await Swal.fire({
        title: `¿Desea ${estado} el Rq de Reposición(TI)?`,
        icon: 'question',
  ...
  fnImprimirRq(){
    ...
    document.title = 'Reporte Requerimiento de Reposición(TI)';
    // Impresion
    setTimeout(()=>{
      window.print();
  ...
  */
  ...
  async fnImprimirCelularReporteRq() {
    ...
    doc.write(divText);
    doc.close();
    doc.title = 'Reporte Requerimiento de Reposición';
  }
