...
@Component({
  selector: "app-orden-sc-aud-edit.component",
  templateUrl: "./orden-sc-aud-edit.component.html",
  styleUrls: ["./orden-sc-aud-edit.component.css"],
  providers: [
    { provide: MAT_DATE_FORMATS, useValue: DD_MM_YYYY_Format },
    DatePipe,
  ],
})
export class OrdenScAudEditComponent implements OnInit {
  // ELIMINAR INICIO
  ListaCiudadAll: any[] = [];
  ListaPartidaAll: any[] = [];
  ...
  async fnFiltroCotizaciones() {
    ...
    await this.service.fnDatosOrdenCompras(this.utilitarios.pOpcion, this.utilitarios.pParametro)
      .subscribe((data: any) => {
  ...
  eventoEnterPorcentajeServicio() {
    ...
    let activoCheckServicioSoles = this.formularioCompras.controls["importeServicioImpuesto"].value;
    let servicioSoles = (activoCheckServicioSoles * (totalSoles / 100)).toFixed(2);
  ...
  obtenerImporteTotalLineas() {
    ...
    this.tempData.map((item) => {
      sutotalAcumulado = sutotalAcumulado + item.precioProv * item.cantidad;
    });
  ...
  // Metodos para los filtros de busquedas 1.Sucusales - Inicio => Partida => ArticuloServicio - Inicio
  ...
  async save() {
    ...
    await Swal.fire({
      ...
      this.ordenCompraSc.tipoDocumento = 'A';

      this.ordenCompraSc.detalle = lineas.join("/");
      // console.log("Enviar");
      // console.log(this.ordenCompraSc);      
